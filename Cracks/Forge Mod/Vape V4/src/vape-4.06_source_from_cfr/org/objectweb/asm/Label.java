...
public class Label {
    ...
    final void accept(MethodVisitor methodVisitor, boolean visitLineNumbers) {
        ...
        if (visitLineNumbers && this.lineNumber != 0) {
            ...
            if (this.otherLineNumbers != null) {
                for (int i10 = 1; i10 <= this.otherLineNumbers[0]; ++i10) {
                    methodVisitor.visitLineNumber(this.otherLineNumbers[i10], this);
                }
            }
        }
    }
    ...
    final boolean resolve(byte[] code, int bytecodeOffset) {
        ...
        for (int i10 = this.forwardReferences[0]; i10 > 0; i10 -= 2) {
            int sourceInsnBytecodeOffset = this.forwardReferences[i10 - 1];
            int reference = this.forwardReferences[i10];
