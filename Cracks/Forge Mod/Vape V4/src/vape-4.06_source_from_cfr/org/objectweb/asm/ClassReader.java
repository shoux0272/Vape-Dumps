...
public class ClassReader {
    ...
    private static byte[] readStream(InputStream inputStream, boolean close) throws IOException {
        ...
        try {
            ...
            try {
                ...
                outputStream.flush();
                byte[] arrby = outputStream.toByteArray();
                return arrby;
            }
    ...
    public String[] getInterfaces() {
        ...
        if (interfacesCount > 0) {
            ...
            for (int i10 = 0; i10 < interfacesCount; ++i10) {
                interfaces[i10] = this.readClass(currentOffset += 2, charBuffer);
            }
        }
        return interfaces;
    }
    ...
    public void accept(ClassVisitor classVisitor, Attribute[] attributePrototypes, int parsingOptions) {
        ...
        for (int i10 = 0; i10 < interfaces.length; ++i10) {
            interfaces[i10] = this.readClass(currentOffset, charBuffer);
            currentOffset += 2;
        }
        ...
        for (int i11 = this.readUnsignedShort(currentAttributeOffset - 2); i11 > 0; --i11) {
    ...
    private void readModuleAttributes(ClassVisitor classVisitor, Context context, int moduleOffset, int modulePackagesOffset, String moduleMainClass) {
        ...
        while (exportsCount-- > 0) {
            ...
            if (exportsToCount != 0) {
                ...
                for (int i10 = 0; i10 < exportsToCount; ++i10) {
                    exportsTo[i10] = this.readModule(currentOffset, buffer);
                    currentOffset += 2;
                }
            }
        ...
        while (opensCount-- > 0) {
            ...
            if (opensToCount != 0) {
                ...
                for (int i11 = 0; i11 < opensToCount; ++i11) {
                    opensTo[i11] = this.readModule(currentOffset, buffer);
                    currentOffset += 2;
                }
            }
        ...
        while (providesCount-- > 0) {
            ...
            for (int i12 = 0; i12 < providesWithCount; ++i12) {
                providesWith[i12] = this.readClass(currentOffset, buffer);
                currentOffset += 2;
            }
    ...
    private int readRecordComponent(ClassVisitor classVisitor, Context context, int recordComponentOffset) {
        ...
        int runtimeInvisibleTypeAnnotationsOffset = 0;
        Attribute attributes = null;
        int attributesCount = this.readUnsignedShort(currentOffset += 4);
        currentOffset += 2;
    ...
    private int readMethod(ClassVisitor classVisitor, Context context, int methodInfoOffset) {
        ...
        while (attributesCount-- > 0) {
            ...
            if ("Code".equals(attributeName)) {
                ...
                for (int i10 = 0; i10 < exceptions.length; ++i10) {
                    exceptions[i10] = this.readClass(currentExceptionOffset, charBuffer);
    ...
    private void readCode(MethodVisitor methodVisitor, Context context, int codeOffset) {
        ...
        while (currentOffset < bytecodeEndOffset) {
            ...
            switch (opcode) {
                ...
                case 170: {
                    ...
                    for (int i10 = 0; i10 < table.length; ++i10) {
                        table[i10] = labels[currentBytecodeOffset + this.readInt(currentOffset)];
                        currentOffset += 4;
                    }
                ...
                case 171: {
                    ...
                    for (int i11 = 0; i11 < numPairs; ++i11) {
                        keys[i11] = this.readInt(currentOffset);
                        values[i11] = labels[currentBytecodeOffset + this.readInt(currentOffset + 4)];
                        currentOffset += 8;
                    }
                ...
                case 186: {
                    ...
                    for (int i12 = 0; i12 < bootstrapMethodArguments.length; ++i12) {
                        bootstrapMethodArguments[i12] = this.readConst(this.readUnsignedShort(bootstrapMethodOffset), charBuffer);
        ...
        if (localVariableTableOffset != 0 && (context.parsingOptions & 2) == 0) {
            ...
            while (localVariableTableLength-- > 0) {
                ...
                String signature = null;
                if (typeTable != null) {
                    for (int i13 = 0; i13 < typeTable.length; i13 += 3) {
                        if (typeTable[i13] != startPc || typeTable[i13 + 1] != index) continue;
                        signature = this.readUTF8(typeTable[i13 + 2], charBuffer);
                        break;
                    }
                }
        ...
        if (visibleTypeAnnotationOffsets != null) {
            ...
            int n6 = typeTable.length;
            for (int k = 0; k < n6; ++k) {
                int typeAnnotationOffset = typeTable[k];
    ...
    private int[] readTypeAnnotations(MethodVisitor methodVisitor, Context context, int runtimeTypeAnnotationsOffset, boolean visible) {
        ...
        currentOffset += 2;
        for (int i10 = 0; i10 < typeAnnotationsOffsets.length; ++i10) {
            typeAnnotationsOffsets[i10] = currentOffset;
        ...
        switch (targetType >>> 24) {
            ...
            case 65: {
                ...
                for (int i10 = 0; i10 < tableLength; ++i10) {
                    ...
                    currentOffset += 6;
                    context.currentLocalVariableAnnotationRangeStarts[i10] = this.createLabel(startPc, context.currentMethodLabels);
                    context.currentLocalVariableAnnotationRangeEnds[i10] = this.createLabel(startPc + length, context.currentMethodLabels);
                    context.currentLocalVariableAnnotationRangeIndices[i10] = index;
                }
                break;
            }
            case 71: 
    ...
    private void readParameterAnnotations(MethodVisitor methodVisitor, Context context, int runtimeParameterAnnotationsOffset, boolean visible) {
        ...
        for (int i10 = 0; i10 < numParameters; ++i10) {
            ...
            while (numAnnotations-- > 0) {
                String annotationDescriptor = this.readUTF8(currentOffset, charBuffer);
                currentOffset += 2;
                currentOffset = this.readElementValues(methodVisitor.visitParameterAnnotation(i10, annotationDescriptor, visible), currentOffset, true, charBuffer);
            }
        }
    }
                ...
                switch (this.classFileBuffer[currentOffset] & 0xFF) {
                    case 66: {
                        ...
                        for (int i10 = 0; i10 < numValues; ++i10) {
                            byteValues[i10] = (byte)this.readInt(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]);
                            currentOffset += 3;
                        }
                    ...
                    case 90: {
                        ...
                        for (int i11 = 0; i11 < numValues; ++i11) {
                            booleanValues[i11] = this.readInt(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]) != 0;
                            currentOffset += 3;
                        }
                    ...
                    case 83: {
                        ...
                        for (int i12 = 0; i12 < numValues; ++i12) {
                            shortValues[i12] = (short)this.readInt(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]);
                            currentOffset += 3;
                        }
                    ...
                    case 67: {
                        ...
                        for (int i13 = 0; i13 < numValues; ++i13) {
                            charValues[i13] = (char)this.readInt(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]);
                            currentOffset += 3;
                        }
                    ...
                    case 73: {
                        ...
                        for (int i14 = 0; i14 < numValues; ++i14) {
                            intValues[i14] = this.readInt(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]);
                            currentOffset += 3;
                        }
                    ...
                    case 74: {
                        ...
                        for (int i15 = 0; i15 < numValues; ++i15) {
                            longValues[i15] = this.readLong(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]);
                            currentOffset += 3;
                        }
                    ...
                    case 70: {
                        ...
                        for (int i16 = 0; i16 < numValues; ++i16) {
                            floatValues[i16] = Float.intBitsToFloat(this.readInt(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]));
                            currentOffset += 3;
                        }
                    ...
                    case 68: {
                        ...
                        for (int i17 = 0; i17 < numValues; ++i17) {
                            doubleValues[i17] = Double.longBitsToDouble(this.readLong(this.cpInfoOffsets[this.readUnsignedShort(currentOffset + 1)]));
                            currentOffset += 3;
                        }
    ...
    private int[] readBootstrapMethodsAttribute(int maxStringLength) {
        ...
        for (int i10 = this.readUnsignedShort(currentAttributeOffset - 2); i10 > 0; --i10) {
            String attributeName = this.readUTF8(currentAttributeOffset, charBuffer);
            int attributeLength = this.readInt(currentAttributeOffset + 2);
            currentAttributeOffset += 6;
            if ("BootstrapMethods".equals(attributeName)) {
                ...
                for (int j10 = 0; j10 < currentBootstrapMethodOffsets.length; ++j10) {
                    currentBootstrapMethodOffsets[j10] = currentBootstrapMethodOffset;
    ...
    public long readLong(int offset) {
        long l12 = this.readInt(offset);
        long l02 = (long)this.readInt(offset + 4) & 0xFFFFFFFFL;
        return l12 << 32 | l02;
    }
    ...
    private ConstantDynamic readConstantDynamic(int constantPoolEntryIndex, char[] charBuffer) {
        ...
        for (int i10 = 0; i10 < bootstrapMethodArguments.length; ++i10) {
            bootstrapMethodArguments[i10] = this.readConst(this.readUnsignedShort(bootstrapMethodOffset), charBuffer);
            bootstrapMethodOffset += 2;
        }
