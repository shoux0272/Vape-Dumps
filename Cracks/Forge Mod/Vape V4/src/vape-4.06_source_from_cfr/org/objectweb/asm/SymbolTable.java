...
final class SymbolTable {
    ...
    private void copyBootstrapMethods(ClassReader classReader, char[] charBuffer) {
        ...
        for (int i10 = classReader.readUnsignedShort(currentAttributeOffset - 2); i10 > 0; --i10) {
        ...
        if (this.bootstrapMethodCount > 0) {
            ...
            for (int i11 = 0; i11 < this.bootstrapMethodCount; ++i11) {
                ...
                this.add(new Entry(i11, 64, offset, hashCode & Integer.MAX_VALUE));
            }
        }
    }
    ...
    private Entry put(Entry entry) {
        if (this.entryCount > this.entries.length * 3 / 4) {
            ...
            for (int i10 = currentCapacity - 1; i10 >= 0; --i10) {
                Entry currentEntry = this.entries[i10];
    ...
    Symbol addBootstrapMethod(Handle bootstrapMethodHandle, Object ... bootstrapMethodArguments) {
        ...
        for (int i10 = 0; i10 < numBootstrapArguments; ++i10) {
            bootstrapMethodArgumentIndexes[i10] = this.addConstant((Object)bootstrapMethodArguments[i10]).index;
        }
        ...
        for (int i11 = 0; i11 < numBootstrapArguments; ++i11) {
            bootstrapMethodsAttribute.putShort(bootstrapMethodArgumentIndexes[i11]);
        }
        ...
        while (entry != null) {
            if (entry.tag == 64 && entry.hashCode == hashCode) {
                ...
                for (int i10 = 0; i10 < length; ++i10) {
                    if (bootstrapMethodsData[offset + i10] == bootstrapMethodsData[otherOffset + i10]) continue;
