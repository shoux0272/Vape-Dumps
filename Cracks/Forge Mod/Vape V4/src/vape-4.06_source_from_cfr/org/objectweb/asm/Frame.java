...
class Frame {
    ...
    final void setInputFrameFromApiFormat(SymbolTable symbolTable, int numLocal, Object[] local, int numStack, Object[] stack) {
        int inputLocalIndex = 0;
        for (int i10 = 0; i10 < numLocal; ++i10) {
            this.inputLocals[inputLocalIndex++] = Frame.getAbstractTypeFromApiFormat(symbolTable, local[i10]);
            if (local[i10] != Opcodes.LONG && local[i10] != Opcodes.DOUBLE) continue;
            this.inputLocals[inputLocalIndex++] = 0x400000;
        }
        ...
        int numStackTop = 0;
        for (int i11 = 0; i11 < numStack; ++i11) {
            if (stack[i11] != Opcodes.LONG && stack[i11] != Opcodes.DOUBLE) continue;
            ++numStackTop;
        }
        this.inputStack = new int[numStack + numStackTop];
        int inputStackIndex = 0;
        for (int i12 = 0; i12 < numStack; ++i12) {
            this.inputStack[inputStackIndex++] = Frame.getAbstractTypeFromApiFormat(symbolTable, stack[i12]);
            if (stack[i12] != Opcodes.LONG && stack[i12] != Opcodes.DOUBLE) continue;
    ...
    private int getInitializedType(SymbolTable symbolTable, int abstractType) {
        if (abstractType == 0x400006 || (abstractType & 0xFFC00000) == 0xC00000) {
            for (int i10 = 0; i10 < this.initializationCount; ++i10) {
                int initializedType = this.initializations[i10];
                ...
                return 0x800000 | symbolTable.addType(symbolTable.getType((int)(abstractType & 1048575)).value);
            }
        }
        return abstractType;
    }
    ...
    final boolean merge(SymbolTable symbolTable, Frame dstFrame, int catchTypeIndex) {
        ...
        for (i11 = 0; i11 < numLocal; ++i11) {
            int concreteOutputType = this.outputLocals != null && i11 < this.outputLocals.length ? ((abstractOutputType = this.outputLocals[i11]) == 0 ? this.inputLocals[i11] : this.getConcreteOutputType(abstractOutputType, numStack)) : this.inputLocals[i11];
            if (this.initializations != null) {
                concreteOutputType = this.getInitializedType(symbolTable, concreteOutputType);
            }
            frameChanged |= Frame.merge(symbolTable, concreteOutputType, dstFrame.inputLocals, i11);
        }
        if (catchTypeIndex > 0) {
            for (i11 = 0; i11 < numLocal; ++i11) {
                frameChanged |= Frame.merge(symbolTable, this.inputLocals[i11], dstFrame.inputLocals, i11);
            }
        ...
        for (i10 = 0; i10 < numInputStack; ++i10) {
            int concreteOutputType = this.inputStack[i10];
            if (this.initializations != null) {
                concreteOutputType = this.getInitializedType(symbolTable, concreteOutputType);
            }
            frameChanged |= Frame.merge(symbolTable, concreteOutputType, dstFrame.inputStack, i10);
        }
        for (i10 = 0; i10 < this.outputStackTop; ++i10) {
            abstractOutputType = this.outputStack[i10];
            int concreteOutputType = this.getConcreteOutputType(abstractOutputType, numStack);
            if (this.initializations != null) {
                concreteOutputType = this.getInitializedType(symbolTable, concreteOutputType);
            }
            frameChanged |= Frame.merge(symbolTable, concreteOutputType, dstFrame.inputStack, numInputStack + i10);
        }
        return frameChanged;
    }
    ...
    final void accept(MethodWriter methodWriter) {
        ...
        for (i10 = 0; i10 < localTypes.length; i10 += (localType = localTypes[i10]) == 0x400004 || localType == 0x400003 ? 2 : 1) {
        ...
        int numStack = 0;
        i10 = 0;
        while (i10 < stackTypes.length) {
            int stackType;
            i10 += (stackType = stackTypes[i10]) == 0x400004 || stackType == 0x400003 ? 2 : 1;
            ++numStack;
        }
        ...
        while (numLocal-- > 0) {
            int localType2;
            i10 += (localType2 = localTypes[i10]) == 0x400004 || localType2 == 0x400003 ? 2 : 1;
    ...
    static void putAbstractType(SymbolTable symbolTable, int abstractType, ByteVector output) {
        ...
        if (arrayDimensions == 0) {
            ...
            if ((abstractType & 0x3C00000) == 0x800000) {
                typeDescriptor.append('L').append(symbolTable.getType((int)(abstractType & 1048575)).value).append(';');
            } else {
